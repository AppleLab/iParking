...
- (void)viewDidLoad
{
   // self.main_map.delegate = self;
    ...
    NSString* filePath = [(AppDelegate*)[myApp delegate] saveFilePath]; // 11111>какая то работа с файлом
...
{
    AppDelegate *delegate = [[UIApplication sharedApplication] delegate];
    FlipsideViewController *controller = delegate.flipsideViewController;// вызов контрллера для xib file
...
- (IBAction)dropPin:(id)sender
{
    ...
    MapNoteAnnotation* pin = [[MapNoteAnnotation alloc] initWithLocation:current_location];// экземпляра нового файла
    [self.main_map addAnnotation:pin];
...
- (void)mapView:(MKMapView *)mapView annotationView:(MKAnnotationView *)view calloutAccessoryControlTapped:(UIControl *)control
{
    ...
    NoteDetailViewController *controller = [[NoteDetailViewController alloc] initWithNibName:@"NoteDetailViewController" bundle:nil];//вызов еще одного xib файла
    controller.delegate = self;
